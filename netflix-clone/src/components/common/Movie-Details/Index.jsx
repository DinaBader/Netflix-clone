...
function Index() {
  ...
  const fetchInfo = () => {
    return fetch(
      "https://api.themoviedb.org/3/movie/550?api_key=324752feff8b4c90538fe9c50b9e9ad5"
    )
      .then((res) => res.json())
      .then((movieData) => {
        setData(movieData);
  ...
  const fetchCast = () => {
    return fetch(
      "https://api.themoviedb.org/3/movie/550/credits?api_key=324752feff8b4c90538fe9c50b9e9ad5"
    )
      .then((res) => res.json())
      .then((castData) => {
    ...
    <>
      <div className="movie-details">
        ...
        <p className="genre">
          ...
          {checkAge(data.adult)} | {data.runtime && timeConvert(data.runtime)} |
        ...
        <p className="starring">
          ...
          {casts &&
            casts.slice(0, 3).map((cast, index) => (
              <span className="cast-name" key={index}>
                {cast.name}
                {index < 2 ? ", " : ""}{" "}
              </span>
            ))}{" "}
